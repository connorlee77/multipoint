...
def main():
    ...
    with torch.no_grad():
        if args.evaluation:
            ...
            if args.plot:
                write_dir = os.path.join('output_align', os.path.basename(os.path.normpath(args.model_dir)))
        ...
        if args.plot:
            ...
            for i, (optical, thermal,
                ...
                cv2.imwrite(os.path.join(write_dir, 'image_pair.png'), np.hstack([im_optical, im_thermal]))
                ...
                cv2.imwrite(os.path.join(write_dir, '{}_matches.png'.format(str(i).zfill(5))), out_image)
                ...
                matchesMask = (diff < 4.0).astype(int).tolist()
                ...
                cv2.imwrite(os.path.join(write_dir, '{}_matches_refined.png'.format(str(i).zfill(5))), out_image_refined)
